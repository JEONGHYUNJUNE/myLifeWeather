...
export function BudgetSummary() {
  ...
  useEffect(()=>{
    try {
      const stored=JSON.parse(localStorage.getItem("australia-2027-budget-v3")||"null") as Record<string,number>|null;
  ...
  useEffect(()=>{
    if(!ready)return;
    localStorage.setItem("australia-2027-budget-v3",JSON.stringify(Object.fromEntries(budget.map(item=>[item.id,item.amount]))));
    setSaved(true);
