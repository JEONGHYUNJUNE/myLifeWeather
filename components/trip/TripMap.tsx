...
export function TripMap() {
  return <section id="map" className="rounded-[30px] bg-[#0b5360] p-4 text-white shadow-2xl shadow-[#0b5360]/15 sm:p-7">
    <div className="flex items-end justify-between gap-4 px-2 pb-5"><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#9dd5d0]">Illustrated route map</p><h2 className="mt-2 text-2xl font-bold">Sydney에서 Melbourne Park, 다시 Sydney</h2></div><Navigation className="text-[#efbd58]"/></div>
    <div className="relative aspect-[4/5] overflow-hidden rounded-[24px] bg-gradient-to-b from-[#b8d9dc] to-[#dce9df] sm:aspect-[16/9]">
      ...
      {trip.mapStops.map((stop,index) => <a key={stop.name} href={mapHref(stop.coordinates)} target="_blank" rel="noopener noreferrer" aria-label={`${stop.name} Google 지도에서 열기`} className="group absolute z-20 -translate-x-1/2 -translate-y-1/2 touch-manipulation" style={{left:`${points[index].x}%`,top:`${points[index].y}%`}}>
    ...
    <p className="px-2 pt-4 text-xs leading-5 text-white/55">지도와 목록의 장소를 누르면 Google 지도에서 실제 위치가 열립니다. 점선은 Sydney ↔ Melbourne 국내선, 트로피는 Australian Open 경기장입니다.</p>
  </section>;
}
